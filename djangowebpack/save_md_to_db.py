...
def insert_category_id(conn, category_id, created_time):
    insert_sql = r"INSERT INTO api_category(name, create_time, last_modified_time)" \
                 " VALUES ( '" + category_id + "', '" + \
...
def get_category_id(conn, category_id, created_time):
    ...
    """
    query_sql = r"SELECT * from api_category WHERE name='" + category_id + "'"
    # print(query_sql)
...
def get_article(conn, title):
    ...
    :return:
    """
    query_sql = r"SELECT * from api_article WHERE title='" + title + "'"
    cursor = conn.execute(query_sql)
    for row in cursor:
        return "文章在数据中"
    return
...
def insert_article_to_db(conn, article):
    ...
    if not get_article(conn, article.title):
        # add = r"INSERT INTO api_article " \
        #       r"(title, body, created_time, last_modified_time, status, views, likes, topped, category_id, abstract) " \
        #       r"VALUES ( '" + \
        ...
        #                           article.created_time)) + "', '" + article.abstract + "')"
        try:
            conn.execute("INSERT INTO api_article "
                         "(title, body, created_time, last_modified_time, status, views, likes, topped, category_id, abstract) "
                         "VALUES "
                         "(?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
                         [article.title, article.body, article.created_time,
